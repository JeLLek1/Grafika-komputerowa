    ...
    for (size_t i = 1; i < this->resolution; i++) {
        for (size_t j = 0; j < this->resolution; j++) {
            ...
            if (i == 1 || i == this->resolution / 2 + 1) {
                ...
                this->points[(i - 1) % this->resolution][j].setGlVertex();
                if (j + 1 == this->resolution)
                    this->drawSingleTriangle(i, j,
                        (i - 1) % this->resolution, j,
                        this->resolution - i, 0,
                        false, !half, j);
                else
                    this->drawSingleTriangle(i, j,
            ...
            if (i + 1 != this->resolution && i != this->resolution / 2
                ...
                if (j + 1 == this->resolution)
                    this->drawSingleTriangle(i, j,
                        (i + 1) % this->resolution, j,
                        this->resolution - (i + 1) % this->resolution, 0,
                        false, half, j);
                else
                    this->drawSingleTriangle(i, j,
                        (i + 1) % this->resolution, j,
                        (i + 1) % this->resolution, j + 1,
                        false, half, j);
                ...
                if (j + 1 == this->resolution) {
                    this->drawSingleTriangle(i, j,
                        ...
                        0, this->resolution - i, 0, true, half, j);

                } else {
                    this->drawSingleTriangle(i, j, (i + 1) % this->resolution,
                        j + 1, i, j + 1, true, half, j);
                }
            }
        }
    }
}
void Egg::drawSingleTriangle(size_t x0, size_t y0, size_t x1, size_t y1,
    size_t x2, size_t y2, bool typeTriangle, bool half, unsigned no) {

    // rozpoczęcie rysowania trójkątów
    glBegin(GL_TRIANGLES);
    if (half) {
        this->texture->setTexture(no, 0, typeTriangle);
        this->points[x0][y0].setGlVertex();
        this->texture->setTexture(no, 1, typeTriangle);
        this->points[x1][y1].setGlVertex();
        this->texture->setTexture(no, 2, typeTriangle);
        this->points[x2][y2].setGlVertex();
    } else {
        this->texture->setTexture(no, 2, typeTriangle);
        this->points[x2][y2].setGlVertex();
        this->texture->setTexture(no, 1, typeTriangle);
...
Egg::Egg(size_t resolution, Material* material,
    Texture* texture, Vector3<GLfloat> pos,
